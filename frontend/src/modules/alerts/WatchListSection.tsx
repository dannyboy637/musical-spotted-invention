...
export function WatchListSection() {
  ...
  const [dropPctOverride, setDropPctOverride] = useState<string | null>(null)
  const [spikePctOverride, setSpikePctOverride] = useState<string | null>(null)
  const [notes, setNotes] = useState('')
  const [formError, setFormError] = useState<string | null>(null)
  const [drafts, setDrafts] = useState<Record<string, DraftState>>({})
  const [draftsInitialized, setDraftsInitialized] = useState(false)

  const dropPct = dropPctOverride ?? String(alertSettings?.revenue_drop_pct ?? 20)
  const setDropPct = (v: string) => setDropPctOverride(v)
  const spikePct = spikePctOverride ?? String(alertSettings?.item_spike_pct ?? 50)
  const setSpikePct = (v: string) => setSpikePctOverride(v)

  if (watchlist && !draftsInitialized) {
